                ...
                <div class="col-lg-12">
                    ...
                    <?php
                        if(isset($_GET['hapus'])){
                            ...
                            $cek = mysql_query("SELECT * FROM kategori WHERE id_kategori='$id_kategori'");
                        
                            if(mysql_num_rows($cek) == 0){
                                echo "<script>alert('Data tidak ditemukan');document.location='kategori_edit.php'</script>";
                            }else{
                                
                                $delete = mysql_query("DELETE FROM kategori WHERE id_kategori='$id_kategori'");
                    
                                if($delete){
                                    mysql_query("ALTER TABLE kategori DROP id_kategori");
                                    mysql_query("ALTER TABLE kategori ADD  id_kategori INT( 11 ) NOT NULL AUTO_INCREMENT FIRST ,ADD PRIMARY KEY (id_kategori)");
                                    echo "<script>swal('Terhapus!', 'Data Kategori berhasil dihapus!', 'success')</script>";
                                }else{
                                    echo "<script>alert('Data gagal terhapus');document.location='kategori_edit.php'</script>";
                                }
                            }
                        }
                    ?>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                    ...
                                    <?php
                                        ...
                                        while ($data = mysql_fetch_array($query)) {
										 $nm_kategori = preg_replace("/\s/","-",$data['nm_kategori']);
										 $url_link = "Kategori-".$data['id_kategori']."-".$nm_kategori.".html";
										 
                                         echo '<tr class="odd gradeX">';
                                         echo "<td>".$data['id_kategori']."</td>";
                                         echo "<td>".$data['nm_kategori']."</td>";
                                         echo "<td>".$data['deskripsi']."</td>";
                                         echo "<td><center><a href = \"".$url_link."\"><span class='label label-warning'>Edit</span> </a>";
										?>	
										<a href ="kategori_edit.php?hapus&id_kategori=<?php echo $data['id_kategori']; ?>" class="delete-link" ><span class="label label-danger">Hapus</span> </a> </td></center>
									<?php } ?>
                                    </tbody>
                                </table>
                            </div>
	...
	<script>
		...
		var getLink = $(this).attr('href');
			swal({
				...
				text: 'Cek kembali , sebelum menghapus data kategori',
				type: 'warning',
				html: true,
